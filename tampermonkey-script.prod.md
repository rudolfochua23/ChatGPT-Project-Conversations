...
(() => {
  ...
  // ━━━ CONFIG ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  const API_URL = "https://chatpile.powerbyte.app";
  ...
  async function extractFilesFromElement(el) {
    ...
    for (const card of fileCards) {
      // Extract file name from card text
      ...
      // Try to find a download link inside the card
      ...
      // Try ChatGPT backend API — extract file ID from card attributes or parent
  ...
  function extractFileIdFromCard(card) {
    // Look for file ID in onclick handlers, href fragments, or nearby elements
    const html = card.outerHTML || "";
    const m = html.match(/file[-_](?:id|ID)['":\s]+['"]?([a-zA-Z0-9_-]{20,})/);
    if (m) return m[1];
    // Check ancestor anchors
